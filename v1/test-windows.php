<?php
    ...
    if($_POST["request"] == "allMoviesAndBonus") {
        $query = "SELECT id, title, version, releasedate, popcornbonus, popcornbonustitle, popcornbonusdesc, active FROM movies WHERE popcornactive = 1 OR popcornbonus > 0";
        if($_POST[device] == "AppleTV") {
            $query = "SELECT id, title, version, releasedate, popcornbonus, popcornbonustitle, popcornbonusdesc, active FROM movies WHERE (popcornactive = 1 OR popcornbonus > 0) AND excludetv = 0";
        }
        if($_POST["mode"] == "dev") {
            $query = "SELECT id, title, version, releasedate, popcornbonus, popcornbonustitle, popcornbonusdesc, active FROM movies WHERE 1";
        }
	...
	if($_POST["request"] == "multiGameList") {
		$games = array();
		$query = "SELECT gid, bet, playerOne, playerTwo, scoreOne, scoreTwo, status, status_updated, timer, questions FROM pt_games WHERE (playerOne = $_POST[uid] OR playerTwo = $_POST[uid]) AND status < 5 AND category = 0 AND challenge = 0 ORDER BY bet DESC";
		$result = mysql_query($query) or die ('error: '. mysql_error());
		$now = time();
	...
	if($_POST["request"] == "multiGameListv2") {
		$data = array();
		$games = array();
		$query = "SELECT gid, bet, playerOne, playerTwo, scoreOne, scoreTwo, status, status_updated, timer, questions FROM pt_games WHERE (playerOne = $_POST[uid] OR playerTwo = $_POST[uid]) AND status < 5 AND status > 0 AND challenge = 0 ORDER BY timer";
		$result = mysql_query($query) or die ('error: '. mysql_error());
		$now = time();
		...
		$games = array();
		$query = "SELECT gid, bet, playerOne, playerTwo, scoreOne, scoreTwo, status, status_updated, timer, questions FROM pt_games WHERE (playerOne = $_POST[uid] OR playerTwo = $_POST[uid]) AND status = 0 AND challenge = 0 ORDER BY bet DESC";
		$result = mysql_query($query) or die ('error: '. mysql_error());
		$now = time();
		...
		//MY CHALLENGES SENT
		$query = "SELECT gid, uid, image, bet, pt_games.timestamp, pt_games.category, challenge_categories.category AS categoryname, playerOne, first_name, last_name, stats_games, stats_wins, stats_points, stats_popcorn FROM pt_games LEFT JOIN pt_multiusers ON pt_games.challenge = pt_multiusers.uid LEFT JOIN challenge_categories ON pt_games.category = challenge_categories.categoryid WHERE playerOne = $_POST[uid] AND challenge != 0 AND status = 0 ORDER BY timestamp";
		$result = mysql_query($query) or die ('error: '. mysql_error());
		...
		$categories[] = $allcat;
		$query = "SELECT categoryid, category, maxbet FROM challenge_categories WHERE active = 1 ORDER BY category";
		$result = mysql_query($query) or die ('error: '. mysql_error());
	...
	if($_POST["request"] == "multiGameStart") {
		$query = "SELECT gid, questions, first_name, last_name, image, token FROM pt_games LEFT JOIN pt_multiusers ON pt_games.playerOne = pt_multiusers.uid WHERE bet = $_POST[bet] AND playerOne != $_POST[uid] AND playerTwo = 0 AND category = 0 AND ((SELECT player_cp FROM pt_multiusers WHERE uid = $_POST[uid]) + player_cp) < 2";
	...
	if($_POST["request"] == "multiGameChallengeDecline") {
		$query = "UPDATE pt_games SET status = 5, status_updated = $_POST[uid] WHERE gid = $_POST[gid]";
		mysql_query($query) or die ('error: '. mysql_error());
	}
	...
	if($_POST["request"] == "multiGameStartv2") {
		...
		$query = "SELECT gid, questions, first_name, last_name, image, token FROM pt_games LEFT JOIN pt_multiusers ON pt_games.playerOne = pt_multiusers.uid WHERE bet = $_POST[bet] AND category = $_POST[category] AND playerOne != $_POST[uid] AND playerTwo = 0";
		$result = mysql_query($query) or die ('error: '. mysql_error());
		$count = mysql_num_rows($result);
		$now = time();
	...
	if($_POST["request"] == "multiGameViewed") {
		...
		$query = "UPDATE pt_multiusers SET stats_wins = stats_wins + $_POST[win], stats_points = (((stats_points * stats_games) + $_POST[points]) / (stats_games + 1)), stats_popcorn = stats_popcorn + $_POST[balance], stats_games = stats_games + 1 WHERE uid = $_POST[uid]";
		mysql_query($query) or die ('error: '. mysql_error());
    ...
    if($_POST["request"] == "multiGameStats") {
		$query = "SELECT uid, first_name, last_name, image, stats_games, stats_wins, stats_points, stats_popcorn FROM pt_multiusers WHERE uid = $_POST[uid]";
		$result = mysql_query($query) or die ('error: '. mysql_error());
		$row = mysql_fetch_array($result);
	...
	if($_POST["request"] == "multiGameLeaderboard") {
		...
		if($_POST[boardtype] == "sp") {
			$query = "SELECT uid, first_name, last_name, image, stats_popcorn, stats_games, stats_sp_points, stats_wins FROM pt_multiusers WHERE 1 ORDER BY stats_sp_points DESC LIMIT $_POST[start], 10";
		} else if($_POST[boardtype] == "wins") {
			$query = "SELECT uid, first_name, last_name, image, stats_popcorn, stats_games, stats_sp_points, stats_wins FROM pt_multiusers WHERE stats_games > 0 ORDER BY stats_wins DESC LIMIT $_POST[start], 10";
		} else {
			$query = "SELECT uid, first_name, last_name, image, stats_popcorn, stats_games, stats_sp_points, stats_wins FROM pt_multiusers WHERE stats_games > 0 ORDER BY stats_popcorn DESC LIMIT $_POST[start], 10";
		}
...
if ($_POST['request'] == "addWindowsToken"){
	...
	$count=mysql_query($query) or die ('error: '. mysql_error()); 
	echo(json_encode(array("response"=>"All Good Bro","data"=>$token." ".$uid)));
}

if ($_POST['request'] == "tellMeAboutWindows") {
	$who = $_POST['uid'];
	$query = "SELECT token FROM pt_multiusers WHERE uid = $who";
	$rows=mysql_query($query) or die ('error: '. mysql_error());
	$user="";
	...
	$auth = authorizeWindows();
	$result=sendMessageWindows($auth,$user,$message);
	echo(json_encode(array("auth"=>$auth,"uri"=>$user,"data"=>$result)));	
