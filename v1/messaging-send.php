...
if($_POST[token] && $_POST[message]) {
    if($_POST[device] == "android") {
        ...
        sendMessageApple($_POST[token], $_POST[message], $_POST[device]);
    }
    echo 'done?';
}
function sendMessageApple($token, $message, $device) {
    ...
    if($device == "iphone") {
        $apnsCert = 'cert-prod-iphone.pem';
    }
    
    if($device == "ipad") {
        $apnsCert = 'cert-prod-ipad.pem';
    }
