...
<?php
    ...
    $query = "SELECT * FROM users WHERE device LIKE 'iPhone%' AND token != ''";
    $result = mysql_query($query) or die ('error: '. mysql_error());
    
    $tokensiphone = 'var tokens = new Array(';
    while($row = mysql_fetch_array($result)) {
        $tokensiphone .= '"'. $row[token] .'", ';
    }
    $tokensiphone = substr($tokensiphone, 0, -2);
    $tokensiphone .= ');';

    
    $query = "SELECT * FROM users WHERE device LIKE 'iPad%' AND token != ''";
    $result = mysql_query($query) or die ('error: '. mysql_error());
    
    $tokensipad = 'var tokens = new Array(';
    while($row = mysql_fetch_array($result)) {
        $tokensipad .= '"'. $row[token] .'", ';
    }
    $tokensipad = substr($tokensipad, 0, -2);
    $tokensipad .= ');';
    
    $query = "SELECT * FROM users WHERE platform = 1 AND token != ''";
    $result = mysql_query($query) or die ('error: '. mysql_error());
...
$(document).ready(function(){
    $('#send-iphone').click(function() {
        <?php echo $tokensiphone;?>
        for(var i=0; i<tokens.length; i++) {
            var token = tokens[i];
            var url = "messaging-send.php";
            var params = {
                device: "iphone",
                token: token,
                message: $('#message').val()
...
<div class="wrapper">
    ...
    <div style="width: 1000px; margin: 0 auto; padding: 50px;">
        ...
        <h3 id="send-ipad">Send Message to all iPads <span id="update-ipad" style="padding 0 0 0 20px; font-style: italic;"></span></h3>
        <h3 id="send-iphone">Send Message to all iPhones <span id="update-iphone" style="padding 0 0 0 20px; font-style: italic;"></span></h3>
