...
<div class="wrapper">
    ...
    <div class="grad1 rounded module">
    	<div style="padding: 0 0 20px 0;"><strong><a href="/appstats/index.php">Home</a> / SuperFan Writeups</strong></div>
        
<?php
$myfile = fopen("used-superfans.txt", "r");
$used = fgets($myfile);
fclose($myfile);
	...
	while($row = $stm->fetch(PDO::FETCH_ASSOC)) {
		...
		if(file_exists($snap)) {
			...
			if(strpos($used,$needle) === FALSE) {
				...
				echo '<div style="padding: 0 0 5px 0;"><input type="checkbox" class="usedbox" id="'. $needle .'" /> '. $row[SuperFanName] .'</div>';
				echo '<div style="padding: 0 0 25px 0;"><img src="http://api.customplay.com/data-tv/'. $movie[id] .'/AppleTVAssets/frame_'. $row[SnapShotFrame] .'.jpg" width="360" height="auto"></div>';
				echo '<div style="padding: 0 0 5px 0; font-style: italic;">SuperFan Fact: '. $title .' - '. $row[SuperFanName] .'<br>'. $row[WriteUp] .'</div>';
				echo '</div>';
			}
