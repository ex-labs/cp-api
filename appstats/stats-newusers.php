...
<div class="wrapper">
    ...
    <div class="grad1 rounded module">
    	<div style="padding: 0 0 20px 0;"><strong><a href="/appstats/index.php">Home</a> / New User Acquisition</strong></div>
		...
		<table class="tablesorter" border="0" cellpadding="0" cellspacing="0">
            ...
            <tr>
                <th>&nbsp;</th>
                <th colspan="5"><div style="text-align:center;">Android</div></th>
                <th colspan="5"><div style="text-align:center;">Apple</div></th>
                <th colspan="5"><div style="text-align:center;">Windows</div></th>
                <th colspan="5"><div style="text-align:center;">Total</div></th>
            </tr>
            <tr>
                <th width="70">Date</th>
...
for($i=0; $i<120; $i++) {
	...
	echo '<tr><td style="border-right: 1px solid #8c8c8c;"><span style="color:'. $rowbg .'; font-weight:bold;">'. date("m/d/Y", $thisDay) .'</span></td><td>'. $count_android_all .'</td><td>'. $count_android_active .'</td><td>'. number_format($ratio) .'%</td><td>';
	...
	if(($costs[windows] + $costs[windows_fb]) > 0) {
		$number = ($costs[windows] + $costs[windows_fb]) / $count_windows_active;
		echo '$'. ($costs[windows] + $costs[windows_fb]) .'</td><td style="border-right: 1px solid #8c8c8c;">$'. number_format($number, 2, '.', '') .'</td>';
	} else {
		echo '$0.00</td><td style="border-right: 1px solid #8c8c8c;">N/A</td>';
	}
	...
	if(($costs[android_fb] + $costs[apple_fb] + $costs[apple_apple] + $costs[windows]) > 0) {
		$number = ($costs[android_fb] + $costs[apple_fb] + $costs[apple_apple] + $costs[windows] + $costs[windows_fb]) / $count_active;
		echo '$'. ($costs[android_fb] + $costs[apple_fb] + $costs[apple_apple] + $costs[windows] + $costs[windows_fb]) .'</td><td>$'. number_format($number, 2, '.', '') .'</td>';
	} else {
	...
	$total_windows_cost = $total_windows_cost + $costs[windows] + + $costs[windows_fb];
