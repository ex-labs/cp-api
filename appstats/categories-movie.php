...
<div class="wrapper">
    ...
    <div style="padding: 0 0 20px 0;"><strong><a href="/appstats/index.php">Home</a> / <a href="/appstats/categories.php">Category Manager</a> / <?php echo urldecode($_GET[title]);?></strong></div>

    <table class="tablesorter" border="0" cellpadding="0" cellspacing="0" style="width: 650px;">
        <thead>
            <tr>
            	<th width="50">ID</th>
...
$stmt = $mysql->query($query);
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
	$subquery = "SELECT * FROM challenge_categories_mov WHERE mid = $_GET[movieid] AND categoryid = $row[categoryid]";
	$stmt2 = $mysql->query($subquery);
	$count = $stmt2->rowCount();
	if($count > 0) {
		echo '<tr><td>'. str_pad($row[categoryid], 2, "0", STR_PAD_LEFT) .'</td><td>'. $row[category] .'</td><td><span style="visibility: hidden;">1</span><input class="updateone" type="checkbox" id="'. $_GET[movieid] .':'. $row[categoryid] .'" checked /></td></tr>';
	} else {
		echo '<tr><td>'. str_pad($row[categoryid], 2, "0", STR_PAD_LEFT) .'</td><td>'. $row[category] .'</td><td><span style="visibility: hidden;">0</span><input class="updateone" type="checkbox" id="'. $_GET[movieid] .':'. $row[categoryid] .'" /></td></tr>';
	}
}
?>
		</tbody>
